...
#####function used for every function add search update
...
def search_data():
    ...
    mycursor.execute(query,(idEntry.get(),nameEntry.get(),emailEntry.get(),phoneEntry.get(),addressEntry.get(),genderEntry.get(),dobEntry.get()))
     ##delete so no previous data appears on screen
    studentTable.delete(*studentTable.get_children())
    ####command already done in mysql we use this command to retrive data to variable
    fetched_data=mycursor.fetchall()
    ###apears data in a row in table
    for data in fetched_data:
        studentTable.insert('',END,values=data)
    #for loop to excess each data ,data can be a list



def add_data():
    #used for error handling if ant textfield id empty show error
    if idEntry.get()=='' or nameEntry.get()=='' or phoneEntry.get()=='' or emailEntry.get()=='' or addressEntry.get()=='' or genderEntry.get()=='' or dobEntry.get()=='':
        ...
        try:
         ...
         #data must be enterd the same order as creation of table

            con.commit()
            result=messagebox.askyesno('Confirm','Data added successfully. Do you want to clean the form?',parent=screen)
            #this message box has yes and no if yes then clear data in gui but not in databse
            if result:
            ...
            messagebox.showerror('Error','WRONG DATA',parent=screen)
            return
...
#########this is the gui of ADD BUTTON


def connect_database():
    def connect():
        global mycursor,con
        #we made it global bcause we will use it in diffrent functions
        try:
        ...
        try:
            ...
            mycursor.execute(query)
            query='create table student(id int not null primary key, name varchar(30),mobile int,email varchar(30),' \
                  'address varchar(100),gender varchar(20),dob DATE,date varchar(50), time varchar(50))'
            mycursor.execute(query)
        except:
            query='use studentmanagementsystem'
            mycursor.execute(query)
